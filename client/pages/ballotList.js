...
export default function ballotList(props) {
    ...
    const ballots = data.map((ballot) => {
        <TouchableOpacity key={ballot._id}
            onPress={() =>
        ...
        <Card key={ballot._id}>
            <View style={{ alignItems: "center", justifyContent: "center" }}>
                    <Text style={{ fontSize: 15 }}>{ballot.status === "active" ?
                        (ballot.hasVoted ? "✅ Status: Voted" : "❌ Status: Has Not Voted") :
                        (ballot.result === "passed" ? "✅ Legislation Passed" : "❌ Failed to Pass")}</Text>
                    ...
                    <Text>
                        {ballot.status === "active" ? `Voting ends ${moment(ballot.endTime).calendar()} (
					${moment(ballot.endTime).fromNow()})` : `Voting ended ${moment(ballot.endTime).calendar()} (
